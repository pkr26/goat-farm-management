...
class TaskCategory(str, enum.Enum):
    ...
    WATER = "WATER"
    # Monthly FAMACHA scoring round (2026-09-23 verification finding: the
    # husbandry plan expects a monthly anemia check, but only the screening
    # pipeline referenced FAMACHA). Generated by the cadence sweep like the
    # other interval rounds; never creatable manually.
    FAMACHA = "FAMACHA"


class ScreeningImageStatus(str, enum.Enum):
    """Lifecycle of one ingested photo.

    PENDING rows are claimed by the next worker cycle; terminal states are
    HEALTHY (gate found nothing — the cascade stops there), FLAGGED (gate saw
    something worth a human look), SKIPPED (never screened: duplicate bytes or
    an unknown farm in the key) and ERROR (download/model failure — retried
    on the next cycle while the row stays PENDING-free, see pipeline).
    """
...
class ScreeningStage(str, enum.Enum):
    """Which cascade step produced a run.

    DETECT splits a multi-goat photo into per-goat crops (Phase 3). GATE is
    the cheap first look every photo/crop gets. CROSS_CHECK is the
    second-provider opinion that runs only on flagged photos (rotation
    Phase 2). SPECIALIST_* runs refine gate observations per body region
    into disease-specific findings.
    """

    DETECT = "DETECT"
    GATE = "GATE"
    CROSS_CHECK = "CROSS_CHECK"
    SPECIALIST_SKIN = "SPECIALIST_SKIN"
    SPECIALIST_EYE = "SPECIALIST_EYE"
    SPECIALIST_HOOF = "SPECIALIST_HOOF"
    SPECIALIST_UDDER = "SPECIALIST_UDDER"
    SPECIALIST_GENERAL = "SPECIALIST_GENERAL"


class ScreeningSeverity(str, enum.Enum):
    """Specialist-assessed severity of a finding (reported, not clinical)."""
...
class ScreeningFindingStatus(str, enum.Enum):
    """Vet review verdict on a model finding. pending_review rows are the

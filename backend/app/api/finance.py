...
async def correct_transaction(
    ...
    async def mutate() -> TransactionOut:
        ...
        if txn.source_type is None and payload.category in SYSTEM_ONLY_CATEGORIES:
            # The creation guard (TransactionIn._manual_row_policy) keeps
            # hand-entered rows out of the system-only categories because a
            # manual row there is indistinguishable from system-generated
            # revenue. A source-less row has no producing record, so a
            # correction re-booking it into one of those categories would
            # mint exactly the forbidden row — while source-linked rows are
            # already frozen to their producing record's type/category by
            # _reconcile_source_record.
            raise HTTPException(
                status_code=422,
                detail=(
                    f"{payload.category} rows are generated by the animal "
                    "purchase/sale workflow and cannot be entered manually"
                ),
            )

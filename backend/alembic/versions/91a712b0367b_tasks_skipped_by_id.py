...
Adversarial audit B8.5c: duties completed via /api/tasks/{id}/complete carry
completed_by_id, but a SKIPPED duty recorded no actor at all. Add the
nullable skipped_by_id FK (users.id), stamped by the skip endpoint; NULL for
service-side skips (abort, kidding leftovers, death/sale) where no single
user made the call. Nullable, so existing rows migrate untouched.
"""

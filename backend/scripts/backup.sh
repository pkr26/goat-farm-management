...
# sidecar has been written.  A directory lock serializes every producer using
...
if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "Backup already running (lock exists: ${LOCK_DIR})" >&2
    exit 3
fi
LOCK_HELD=1
printf '%s\n' "$$" > "${LOCK_DIR}/pid"
...
DB_METADATA="$(printf '%s' "${RAW_DB_URL}" | python3 "${SCRIPT_DIR}/libpq_url.py" "${PGPASSFILE}")"
unset RAW_DB_URL
PG_URL="${DB_METADATA%%$'\n'*}"
DB_NAME="${DB_METADATA#*$'\n'}"
